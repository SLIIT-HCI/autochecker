...
def prep_tests(tt):
    ...
    if run:
        test_in = {}
        test_out = {}
        os.chdir(tt)# modified to tally with the export functions
...
def run_file(filepath, studentid):
    ...
    for key in test_in.keys():
        ...
        p = Popen([output], stdin=PIPE, stdout=PIPE, stderr=STDOUT)#all
...
def compile_file(filepath, studentid):
    ...
    if os.path.exists(output):#for windows, for all
